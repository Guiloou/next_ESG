...
import { useActionState } from "react"; // Certifique-se de que você está usando a versão correta do React
import { criarPost } from "../_servicos/posts-servicos";

function CriarPostBotao({ isPending }) {
  return (
...
export default function CriarPostModal() {
  ...
  const [isPending, setIsPending] = useState(false); // Gerenciando estado de pendência manualmente
  const [state, formAction] = useActionState(
    async (_, formData) => {
      setIsPending(true); // Definindo o estado como pendente
      ...
      if (result?.data) {
        formRef.current.reset(); // Resetando o formulário
      }

      setIsPending(false); // Finalizando estado de pendência
      return result;
    },
    { data: null, erros: [] }
  );
        ...
        <div className="modal-box border border-solid border-zinc-50/[0.12]">
          ...
          <form action={formAction} ref={formRef}>
            ...
            <div className="modal-action">
              <CriarPostBotao isPending={isPending} />
